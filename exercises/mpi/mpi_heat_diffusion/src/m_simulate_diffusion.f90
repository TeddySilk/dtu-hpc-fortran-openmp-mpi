...
CONTAINS
   ...
   ! ------------------------------------------------- !
   SUBROUTINE simulate_diffusion_single(tfield, nsteps, input_step, verbose, binary_backup, benchmarking)

      ! input variables
      REAL, DIMENSION(:, :), INTENT(INOUT) :: tfield
      INTEGER, INTENT(IN) :: nsteps, verbose
      LOGICAL, INTENT(IN) :: binary_backup, benchmarking
      ...
      IF (benchmarking) THEN
         WRITE(string, '(A,A,A)') './res/', trim(benchmark_file), '.dat'
         ...
         ENDIF
         WRITE(300, '(3E12.4)') cpu_t2 - cpu_t1, time_end - time_init, (cpu_t2 - cpu_t1) / (time_end - time_init)
         CLOSE(300)
      ENDIF
   ...
   SUBROUTINE simulate_diffusion_double(tfield, nsteps, input_step, verbose, binary_backup, benchmarking)
