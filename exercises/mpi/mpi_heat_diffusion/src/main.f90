PROGRAM main
   ...
   IF (benchmark) THEN
      DO i = 1, 20
         CALL simulate_diffusion(tfield, nsteps, &
            input_step = step, verbose = 0, &
            binary_backup = save_bin, benchmarking = benchmark)
      ENDDO
   ELSE
      CALL simulate_diffusion(tfield, nsteps, &
         input_step = step, verbose = 0, &
   ...
   ! EXTRACT FIELD(S)                                  !
   ! ------------------------------------------------- !
   !CALL extract_field(tfield, output_file, nsteps + step)
   IF (save_bin) THEN
      CALL extract_binary(tfield, binary_file, nsteps + step)
   ENDIF
